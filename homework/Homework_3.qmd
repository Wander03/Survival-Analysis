...
(a) Construct the likelihood function for $\lambda$, i.e., find $L(\lambda)$, simplifying as much as possible.
    ...
    = (\frac{2}{\lambda^2})^{10} \prod_{1}^{10}[t_i] \times exp[-\sum_1^{10}(\frac{t_i}{\lambda})^2] \\
    = (\frac{2}{\lambda^2})^{10} \prod_{1}^{10}t_i \times exp[-\frac{1}{\lambda^2} \sum_1^{10} t_i^2]
    $$

(b) Find the MLE for $\lambda$.

    $$
    l(\lambda) = ln[(\frac{2}{\lambda^2})^{10} \prod_{1}^{10}[t_i] \times exp[-\frac{1}{\lambda^2} \sum_1^{10} t_i^2] \\
...
(a) Construct the likelihood function for $\lambda$, simplifying as much as possible.
    ...
    $$
    L(\lambda) = \prod_{\text{complete times}} f(t_i) \times \prod_{\text{right censored times}} S(t_i) \\
    = \prod \frac{1}{\lambda}exp[\frac{-t_i}{\lambda}] \times exp[\frac{-t_i}{\lambda}] \\
    = \frac{1}{\lambda^8} exp[-\sum\frac{t_i}{\lambda}] \times exp[-\sum\frac{t_i}{\lambda}] \\
